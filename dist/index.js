...
function importSchema(schema, options) {
    ...
    var schemas = options.schemas, _a = options.mergeableTypes, mergeableTypes = _a === void 0 ? [] : _a;
    var allMergeableTypes = mergeableTypes.concat(rootFields);
    ...
    var _b = collectDefinitions(['*'], sdl, schema, schemas), allDefinitions = _b.allDefinitions, typeDefinitions = _b.typeDefinitions;
    ...
    var firstTypes = ramda_1.flatten(typeDefinitions).filter(function (d) {
        return ramda_1.includes(d.name.value, allMergeableTypes);
    });
    var otherFirstTypes = typeDefinitions[0].filter(function (d) { return !ramda_1.includes(d.name.value, allMergeableTypes); });
    var firstSet = firstTypes.concat(otherFirstTypes);
    var processedTypeNames = [];
    var mergedFirstTypes = [];
    var _loop_1 = function (type) {
        if (!ramda_1.includes(type.name.value, processedTypeNames)) {
    ...
    document = __assign({}, document, { definitions: definition_1.completeDefinitionPool(ramda_1.flatten(allDefinitions), firstSet, ramda_1.flatten(typeDefinitions)) });
...
function collectDefinitions(imports, sdl, filePath, schemas, processedFiles, typeDefinitions, allDefinitions) {
    ...
    rawModules.forEach(function (m) {
        ...
        if (!processedFile || !processedFile.find(function (rModule) { return ramda_1.equals(rModule, m); })) {
...
function filterImportedDefinitions(imports, typeDefinitions, allDefinitions) {
    ...
    if (ramda_1.includes('*', imports)) {
        if (imports.length === 1 &&
            imports[0] === '*' &&
            allDefinitions.length > 1) {
            var previousTypeDefinitions_1 = ramda_1.indexBy(function (def) { return def.name.value; }, ramda_1.flatten(allDefinitions.slice(0, allDefinitions.length - 1)).filter(function (def) { return !ramda_1.includes(def.name.value, rootFields); }));
        ...
        var result = filteredDefinitions.filter(function (d) {
            return ramda_1.includes(d.name.value, imports.map(function (i) { return i.split('.')[0]; }));
        });
        var fieldImports = imports.filter(function (i) { return i.split('.').length > 1; });
        var groupedFieldImports = ramda_1.groupBy(function (x) { return x.split('.')[0]; }, fieldImports);
        var _loop_2 = function (rootType) {
            var fields = groupedFieldImports[rootType].map(function (x) { return x.split('.')[1]; });
            filteredDefinitions.find(function (def) { return def.name.value === rootType; }).fields = filteredDefinitions.find(function (def) { return def.name.value === rootType; }).fields.filter(function (f) { return ramda_1.includes(f.name.value, fields) || ramda_1.includes('*', fields); });
        };
...
function filterTypeDefinitions(definitions) {
    ...
    return definitions
        .filter(function (d) { return ramda_1.includes(d.kind, validKinds); })
